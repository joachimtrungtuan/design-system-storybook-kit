...
import { toolkitRoot } from "../../../engine/src/template/materialise.js";
import { runUpdatePipeline } from "../../../engine/src/update/pipeline.js";
import { formatUpdateReport, writeUpdateReport } from "../../../engine/src/update/report.js";
...
export async function runUpdate(options = {}) {
    ...
    if (options.to !== undefined) {
        throw new ActionableError("The --to flag is not implemented yet.", "Omit --to to update to the toolkit version currently installed in this project.", "docs/update-and-migration.md");
    }
    const onConflict = options.onConflict ?? "skip";
    if (onConflict !== "skip") {
        throw new ActionableError(`--on-conflict=${onConflict} is not implemented yet.`, "Use --on-conflict=skip (the default), which leaves conflicted files untouched for you to merge by hand.", "docs/update-and-migration.md");
    }
    ...
    if (!dryRun) {
        ...
        if (!gitEnvironment.insideRepository) {
            throw new ActionableError("ds update requires a git repository, since a branch is how an update run can be discarded.", "Run 'git init' (or run this command inside the project's existing repository) and try again.", root);
        }
        if (!gitEnvironment.clean) {
            throw new ActionableError("ds update requires a clean working tree.", "Commit or stash pending changes, then run 'ds update' again.", root);
        }
    }
    let branch;
    if (!dryRun) {
        ...
        try {
            ...
            throw new ActionableError(`Could not create branch '${branch}'.`, `Delete or check out any pre-existing branch named '${branch}', then run 'ds update' again.`, root);
        }
    }
    let result;
    try {
        ...
        if (branch === undefined)
            throw error;
        throw new ActionableError(`ds update failed on branch '${branch}': ${error instanceof Error ? error.message : String(error)}`, `Inspect the branch, then discard it with 'git checkout - && git branch -D ${branch}' if you don't want to keep it.`, root);
    }
