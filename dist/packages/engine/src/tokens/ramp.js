import { hexToHsl, hexToOklch, hslToHex, normalizeHex, oklchToHex } from "./color.js";
import { RAMP_STEPS } from "./schema.js";
const LIGHT_ENDPOINT = 0.97;
const OKLCH_DARK_ENDPOINT = 0.18;
const HSL_DARK_ENDPOINT = 0.12;
function curvedProgress(value) {
    return value ** 1.15;
}
function interpolate(start, end, progress) {
    return start + (end - start) * curvedProgress(progress);
}
function lightnessAt(index, anchorIndex, baseLightness, darkEndpoint) {
...
export function generateRamp(definition) {
    ...
    for (const [index, step] of RAMP_STEPS.entries()) {
        ...
        if (hsl === undefined) {
            throw new TypeError(`Unsupported ramp mode: ${definition.$mode}`);
        }
        result[step] = {
            hex: hslToHex({
                ...hsl,
