...
import { TIERS, exportSpecifiers, kebabCase, pascalCase, pascalFromKebab, violation } from "./helpers.js";
export const validateNaming = (snapshot) => {
    const violations = [];
    for (const tier of TIERS) {
        ...
        for (const directory of componentDirectories) {
            ...
            const componentFiles = [...snapshot.files.keys()].filter((path) => dirname(path) === directory && path.endsWith(".tsx"));
            const expectedFile = `${directory}/${expectedComponent}.tsx`;
            if (!snapshot.files.has(expectedFile)) {
                violations.push(violation("V5", expectedFile, `Component directory '${name}' has no matching component source.`, `Create ${expectedComponent}.tsx or remove the empty component directory.`));
            }
            for (const file of componentFiles) {
                const stem = basename(file, ".tsx");
                if (!pascalCase(stem) || stem !== expectedComponent) {
                    violations.push(violation("V5", file, `Component file '${basename(file)}' does not match '${name}'.`, `Rename it to ${expectedComponent}.tsx.`));
                }
            }
            for (const file of snapshot.files.keys()) {
                if (dirname(file) !== directory || !file.endsWith(".ts") || basename(file) === "index.ts")
                    continue;
