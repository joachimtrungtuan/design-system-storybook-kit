...
import { TIERS, defaultMetaObject, nodeLine, objectProperty, pascalFromKebab, resolveModule, storyTier, typeText, violation, } from "./helpers.js";
function identifierValue(property) {
    return property !== undefined && ts.isIdentifier(property.initializer)
        ? property.initializer.text
        : undefined;
}
function referencedComponents(meta) {
    ...
    if (subcomponents !== undefined && ts.isObjectLiteralExpression(subcomponents)) {
        for (const property of subcomponents.properties) {
            if (ts.isShorthandPropertyAssignment(property))
                referenced.add(property.name.text);
...
function storybookTypeNames(source, exportedName) {
    const names = new Set();
    for (const statement of source.statements) {
        if (!ts.isImportDeclaration(statement) || !ts.isStringLiteral(statement.moduleSpecifier) || !/^@storybook\//u.test(statement.moduleSpecifier.text))
            continue;
...
function exactGenericType(text, names) {
    return [...names].some((name) => new RegExp(`^${name}(?:<|$)`, "u").test(text.trim()));
}
function hasMetaTyping(source) {
    ...
    for (const statement of source.statements) {
        ...
        if (ts.isSatisfiesExpression(statement.expression))
            return exactGenericType(typeText(statement.expression.type), metaTypes);
        if (ts.isAsExpression(statement.expression))
            return exactGenericType(typeText(statement.expression.type), metaTypes);
        if (ts.isIdentifier(statement.expression))
            exportedIdentifier = statement.expression.text;
    }
    ...
    for (const statement of source.statements) {
        ...
        for (const declaration of statement.declarationList.declarations) {
            if (!ts.isIdentifier(declaration.name) || declaration.name.text !== exportedIdentifier)
                continue;
            if (exactGenericType(typeText(declaration.type), metaTypes))
                return true;
...
function hasNamedStory(source) {
    const storyTypes = storybookTypeNames(source, "StoryObj");
    if (storyTypes.size === 0)
        return false;
    for (const statement of source.statements) {
        if (!ts.isVariableStatement(statement) || statement.modifiers?.some((modifier) => modifier.kind === ts.SyntaxKind.ExportKeyword) !== true)
            continue;
        for (const declaration of statement.declarationList.declarations) {
            if (exactGenericType(typeText(declaration.type), storyTypes))
                return true;
            const initializer = declaration.initializer;
            if (initializer !== undefined && ts.isSatisfiesExpression(initializer) && exactGenericType(typeText(initializer.type), storyTypes))
                return true;
        }
    }
    return false;
}
function exportedComponentDirectory(snapshot, modulePath, exportedName, visited = new Set()) {
...
function importedOrigins(snapshot, file, source) {
    ...
    for (const statement of source.statements) {
        ...
        if (bindings !== undefined && ts.isNamedImports(bindings)) {
            for (const element of bindings.elements) {
                ...
                if (directory !== undefined)
                    origins.set(element.name.text, directory);
            }
        }
        ...
        if (defaultName !== undefined) {
            ...
            if (directory !== undefined)
                origins.set(defaultName, directory);
        }
    }
    return origins;
}
function componentDirectoryFromOrigin(origin) {
    const match = /^(src\/components\/[^/]+\/[^/]+)(?:\/|$)/u.exec(origin);
    return match?.[1];
}
export const validateStories = (snapshot) => {
    const violations = [];
    const storiesByTier = new Map();
    for (const [file, source] of snapshot.sources) {
        const tier = storyTier(file);
        if (tier === undefined || !/\.stories\.[cm]?[jt]sx?$/u.test(file))
            continue;
        ...
        if (meta === undefined || objectProperty(meta, "title") === undefined || objectProperty(meta, "component") === undefined || !hasMetaTyping(source) || !hasNamedStory(source)) {
            violations.push(violation("V11", file, "Story file must export typed Meta with title and component, plus a named StoryObj story.", "Add a typed default meta object and at least one exported StoryObj."));
            continue;
        }
        ...
        if (title === undefined || !title.startsWith(expectedPrefix) || title.slice(expectedPrefix.length).length === 0) {
            violations.push(violation("V10", file, `Story title must start with '${expectedPrefix}'.`, `Set title to ${expectedPrefix}<Name>.`, titleProperty === undefined ? undefined : nodeLine(snapshot, file, titleProperty)));
        }
    }
    for (const tier of TIERS) {
        ...
        for (const directory of snapshot.directories) {
            ...
            if (!storyCoverage.has(directory)) {
                violations.push(violation("V9", directory, `Component '${expected}' has no declared story coverage in src/stories/${tier}.`, "Declare it as the story component or one of its subcomponents."));
            }
        }
    }
    return violations;
