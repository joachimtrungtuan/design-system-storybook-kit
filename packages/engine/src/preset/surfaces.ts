...
import { generateRamp } from "../tokens/ramp.ts";
import {
  isRampDefinition,
  RAMP_STEPS,
  tokenReferencePath,
...
import { hexToRgb, normalizeHex } from "../tokens/color.ts";
...
function isTokenObject(value: TokenNode | undefined): value is TokenObject {
...
function lookupToken(root: TokenObject, path: string, source: string): TokenNode {
...
function resolveColor(root: TokenObject, reference: string, source: string, seen = new Set<string>()): string {
...
export function classifySurface(hex: string): BrandSurfaceMode {
  return relativeLuminance(hex) > 0.179 ? "light" : "dark";
}

export function buildBrandSurfaces(tokens: ParsedTokens): BrandSurface[] {
