...
async function toolkitRoot(): Promise<string> {
...
async function renderDirectory(directory: string, values: Record<string, string>): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    const rendered = content.replace(PLACEHOLDER, (_, key: string) => values[key] ?? "");
    if (rendered !== content) await writeFile(path, rendered);
  }
}

async function assertNoPlaceholders(directory: string): Promise<void> {
  const entries = await readdir(directory, { withFileTypes: true });
  for (const entry of entries) {
    const path = resolve(directory, entry.name);
    if (entry.isDirectory()) {
