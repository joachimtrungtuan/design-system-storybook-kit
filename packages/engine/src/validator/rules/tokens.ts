import { generateTokensCss } from "../../tokens/codegen.ts";
import type { ProjectSnapshot, ValidatorRule, Violation } from "../types.ts";
...
function sourceFiles(snapshot: ProjectSnapshot): Array<[string, string]> {
  return [...snapshot.files].filter(([path]) =>
    (path.startsWith("src/") || path.startsWith(".storybook/")) &&
    /\.(?:css|mdx|[cm]?[jt]sx?)$/u.test(path) && path !== "src/styles/tokens.css",
  );
}

function stripFontSourceExemptions(content: string): string {
  return content
    .replace(/^\s*@import[^;]+;.*$/gmu, "")
    .replace(/@font-face\s*\{[^}]*\}/gisu, (block) => block.replace(/font-family\s*:[^;]+;/giu, ""));
}

function collectPrimitiveStrings(value: unknown, path: string[] = [], output: Array<{ path: string; value: string }> = []): Array<{ path: string; value: string }> {
  if (typeof value === "string") {
    output.push({ path: path.join("."), value });
    return output;
  }
  if (typeof value !== "object" || value === null || Array.isArray(value)) return output;
  for (const [key, child] of Object.entries(value)) collectPrimitiveStrings(child, [...path, key], output);
  return output;
}

function tokenErrorViolation(snapshot: ProjectSnapshot, ruleId: `V${number}`, pattern: RegExp, action: string): Violation[] {
  const error = snapshot.tokenError;
  if (error === undefined || !pattern.test(error.message)) return [];
  ...
  return [violation(
    ...
    error.message,
    action,
    offset < 0 ? undefined : lineOf(source, offset),
  )];
}

function validateRawValues(snapshot: ProjectSnapshot): Violation[] {
  ...
  for (const [file, original] of sourceFiles(snapshot)) {
    ...
    for (const token of [...fontValues, ...typeScaleValues]) {
      if (token.value.length < 2) continue;
...
function validateComponentColors(snapshot: ProjectSnapshot): Violation[] {
  const violations: Violation[] = [];
  for (const [file, content] of sourceFiles(snapshot)) {
    if (!file.startsWith("src/components/") && !file.startsWith("src/pages/") && !file.startsWith(".storybook/")) continue;
...
function validateArbitraryValues(snapshot: ProjectSnapshot): Violation[] {
  const violations: Violation[] = [];
  const root = snapshot.parsedTokens?.root;
  if (root === undefined) return violations;
  const prefixes = new Set<string>();
  if (root.color !== undefined) ["bg", "text", "border", "fill", "stroke"].forEach((prefix) => prefixes.add(prefix));
  if (root.typography !== undefined) ["text", "font", "tracking", "leading"].forEach((prefix) => prefixes.add(prefix));
  if (root.spacing !== undefined) ["p", "px", "py", "pt", "pr", "pb", "pl", "m", "mx", "my", "mt", "mr", "mb", "ml", "gap", "space-x", "space-y"].forEach((prefix) => prefixes.add(prefix));
  if (root.radius !== undefined) prefixes.add("rounded");
  if (root.shadow !== undefined) prefixes.add("shadow");
  if (root.motion !== undefined) ["duration", "ease"].forEach((prefix) => prefixes.add(prefix));
  if (prefixes.size === 0) return violations;
  const pattern = new RegExp(`\\b(?:${[...prefixes].sort((a, b) => b.length - a.length).join("|")})(?:-[trblxy])?-\\[[^\\]]+\\]`, "gu");
  for (const [file, content] of sourceFiles(snapshot)) {
    for (const match of content.matchAll(pattern)) {
      if (/^bg-\[(?:url\(|image-set\(|(?:repeating-)?(?:linear|radial|conic)-gradient\()/u.test(match[0])) continue;
...
function validateGeneratedCss(snapshot: ProjectSnapshot): Violation[] {
  if (snapshot.parsedTokens === undefined) return [];
  ...
  try {
    ...
    return [violation(
      ...
      "tokens.json",
      message,
      ruleId === "V19" ? "Point semantic colours at existing, non-cyclic token paths." : "Use token groups supported by codegen before regenerating tokens.css.",
    )];
  }
...
function validateGroupNames(snapshot: ProjectSnapshot): Violation[] {
  ...
  for (const key of Object.keys(snapshot.rawTokens)) {
    ...
    if (key.startsWith("$") || TOP_LEVEL_GROUP.test(key)) continue;
    const offset = source.indexOf(`"${key}"`);
...
export const validateTokens: ValidatorRule = (snapshot) => [
