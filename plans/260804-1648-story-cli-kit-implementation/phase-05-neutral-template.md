...
The maintainer chose to mirror `win-ui-layout`. Scouting on 2026-08-04 established what that can and cannot mean.

## What mirroring actually involves

`win-ui-layout` supplies content. It cannot supply structure, because it does not satisfy the contract — it predates it. Every difference below is work in this phase:
...
| brand values throughout | neutral, brand-free but complete | strip WIN Flavor identity, keep the structure |
...
## Token group naming — decided 2026-08-04
...
**This becomes a `[V]` rule in Phase 3:** an unknown top-level group that is neither a Tailwind namespace nor `$`-prefixed is a violation. Contract, validator and template change together in this phase.

## Component set and dependencies — decided 2026-08-04

| Tier | Ship | Leave out, and why |
| --- | --- | --- |
| atoms | `button`, `input`, `textarea`, `badge`, `container`, `heading`, `text`, `image`, `icon`, `sliding-number` | — |
| molecules | `card`, `breadcrumb`, `pagination` | `blog-archive`, `product-archive`, `single-product` — WIN Flavor's domain, not neutral; `splide-carousel` — carousel dependency not adopted |
...
**The type scale is worth lifting wholesale.** `win-ui-layout` has fourteen font-size entries and a `type-scale.ts` helper beside its typography atoms — a real, considered scale. Keep the structure, neutralise the family.
...
- Modify: `INDEX.md`
- Create: `update-logs/2026-08-04/NN-neutral-template.md`
...
3. Neutralise: replace WIN Flavor brand values with a neutral palette, keeping ramp structure, anchors and modes intact.
4. Scaffold the directory layout empty and run `ds validate` against it. Expect a wall of violations; that wall is the phase's task list.
...
10. Full run: install, start Vite, start Storybook, `ds validate`. All four must succeed from a clean copy.
11. Update-log entry; `INDEX.md` and template README sync.

## Success Criteria

- [ ] `ds validate` reports zero violations
- [ ] Vite dev server and Storybook both start from a clean copy of the template
...
- [ ] No WIN Flavor brand value, asset or copy survives anywhere in the tree
